...
def generate_navigation_task_env(WALL = None):
    ...
    # # Setup observation space from the Environment space - CHANGE IN TaskEnvironmentGym such that self.observation_space allows 50 place cell activities
...
def make(env_id: Environment) -> gym.Env:
    match env_id:
        ...
        case "Ratinabox-v0-xy":
            # Env = Environment() # not gym env ???? - or use TaskEnvironmentGym directly instead of SpatialGoal?
            env = generate_navigation_task_env() # make the task environment and agent
            print('generated env', env_id)
            return env
        case 'Ratinabox-v0-pc-random-walls95':
